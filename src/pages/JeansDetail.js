...
import { getJeans, getjeansDetails } from "../Actions/shirtAction";
import '../detail.css'
...
const JeansDetail = () => {
  ...
  const {loading,error,product} = useSelector(state =>state.jeansdetail)
  
  const { products } = useSelector(state => state.jeans)
  const { id } = useParams(); 
