...
import { getKurta, getkurtaDetails } from "../Actions/shirtAction";
...
const KurtaDetail = () => {
  ...
  const {loading,error,product} = useSelector(state =>state.kurtadetail)
  const {products } = useSelector(state => state.kurta)
  const { id } = useParams(); 
