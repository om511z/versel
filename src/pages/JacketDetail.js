...
import { getjacketDetails, getJacket } from "../Actions/shirtAction";
import '../detail.css'
...
const JacketDetail = () => {
  ...
  const { loading, error, product } = useSelector(state => state.jacketdetail)
  const { products } = useSelector(state => state.jacket)
  const { id } = useParams();
